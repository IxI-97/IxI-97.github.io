...
BiomiX is developed to deconvolute matrices containing samples from different experiments and select only the ones the user wants to analyse (E.i analyse only B cells in a matrix containig the expression of B cells, T cells, Neutrophiles and Monocytes). This addition aws developed to avoid the sorting of each group of samples in different matrices. The format to respect is clear, generaly we suggest to call samples names using an ID number (e.i 3560087) for the analysis, except when there are multiple samples from the same patients, in this case a single ID number is not enough. To distinguish these multiple samples the format suggested is ID number + tissue_name (e.i 3560087_BLymphocytes). 

The tool has been developed to accept both the formats and to provide an input in MOFA including only the ID number to allow integration with different omics. So if you have convoluted matrix it is enough to run the DGE analysis, cross the selection option and use in the label the name of the tissue. BiomiX will look in the ID_CELL_TYPE column and retrieve the tissue_name filtering for it. If the sample ID is only numberic that's all, otherwise if the sample ID includes number + tissue_name it will be replaced by only the ID number. 
